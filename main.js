...
function setRandomCellOn() {
  ...
  // }
  currentState[63] = Math.random() < 0.05 ? getRandomIntInclusive(100, 119) : 0;
  currentState[64] = Math.random() < 0.05 ? getRandomIntInclusive(100, 119) : 0;
  currentState[65] = Math.random() < 0.05 ? getRandomIntInclusive(100, 119) : 0;
}
...
function doStepSand(i5, s5) {
  ...
  if (s2 === 0) {
    nextState[i2] = s5;
  } if (s2 !== null && materials.get(s5).density > materials.get(s2).density) {
    nextState[i2] = s5;
    nextState[i5] = s2;
  } else if (s1 === 0 && s3 === 0) {
